...
router.get('/dispositivos', autenticar, operational, (req, res) => {
  const db = getDb();
  const rows = req.usuario.perfil === 'admin'
    ? db.prepare('SELECT * FROM dispositivos').all()
    : db.prepare('SELECT * FROM dispositivos WHERE unidade_id = ?').all(req.usuario.unidade_id);
  res.json(rows.map(d => ({
    ...d, total_numeros: db.prepare('SELECT COUNT(*) AS c FROM numeros_dispositivos WHERE dispositivo_id = ?').get(d.id).c
  })));
});

// Gera relatório simples de dispositivos com timestamp de geração
router.get('/dispositivos/relatorio', autenticar, operational, (req, res) => {
  const db = getDb();
  const rows = req.usuario.perfil === 'admin'
    ? db.prepare('SELECT * FROM dispositivos').all()
    : db.prepare('SELECT * FROM dispositivos WHERE unidade_id = ?').all(req.usuario.unidade_id);
...
router.post('/dispositivos', autenticar, operational, (req, res) => {
  ...
  const result = db.prepare('INSERT INTO dispositivos (nome, usuario_id, setor_id, local_id, unidade_id, ativo, status, fabricante, modelo, data_aquisicao, observacoes, criado_em, atualizado_em) VALUES (?,?,?,?,?,1,?,?,?,?,?,?,?)').run(
...
router.put('/dispositivos/:id', autenticar, operational, (req, res) => {
  const db = getDb();
  const item = db.prepare('SELECT * FROM dispositivos WHERE id = ?').get(id(req.params.id));
  ...
  if (sets.length) { sets.push('atualizado_em = ?'); vals.push(now()); vals.push(item.id); db.prepare(`UPDATE dispositivos SET ${sets.join(', ')} WHERE id = ?`).run(...vals); }
  res.json({ sucesso: true, mensagem: 'Dispositivo atualizado!' });
});

// Remove um dispositivo e seus números associados
router.delete('/dispositivos/:id', autenticar, operational, (req, res) => {
  const db = getDb();
  const item = db.prepare('SELECT * FROM dispositivos WHERE id = ?').get(id(req.params.id));
  if (!item) return res.status(404).json({ erro: 'Dispositivo não encontrado.' });
  if (!unitScope(req, item)) return res.status(403).json({ erro: 'Acesso negado.' });
  db.prepare('DELETE FROM numeros_dispositivos WHERE dispositivo_id = ?').run(item.id);
  db.prepare('DELETE FROM dispositivos WHERE id = ?').run(item.id);
  res.json({ sucesso: true, mensagem: 'Dispositivo removido!' });
});

// Lista números telefônicos associados a um dispositivo
router.get('/dispositivos/:id/numeros', autenticar, operational, (req, res) => {
  res.json(getDb().prepare('SELECT * FROM numeros_dispositivos WHERE dispositivo_id = ?').all(id(req.params.id)));
});

// Adiciona um número telefônico a um dispositivo
router.post('/dispositivos/:id/numeros', autenticar, operational, (req, res) => {
  const db = getDb();
  const { numero, observacoes } = req.body;
  const result = db.prepare('INSERT INTO numeros_dispositivos (dispositivo_id, numero, observacoes, criado_em, atualizado_em) VALUES (?, ?, ?, ?, ?)').run(
...
router.put('/dispositivos/:id/numeros/:numeroId', autenticar, operational, (req, res) => {
  const db = getDb();
  const n = db.prepare('SELECT * FROM numeros_dispositivos WHERE id = ?').get(id(req.params.numeroId));
  ...
  if (sets.length) { sets.push('atualizado_em = ?'); vals.push(now()); vals.push(n.id); db.prepare(`UPDATE numeros_dispositivos SET ${sets.join(', ')} WHERE id = ?`).run(...vals); }
  res.json({ sucesso: true, mensagem: 'Número atualizado!' });
});

// Remove um número telefônico de um dispositivo
router.delete('/dispositivos/:id/numeros/:numeroId', autenticar, operational, (req, res) => {
  getDb().prepare('DELETE FROM numeros_dispositivos WHERE id = ?').run(id(req.params.numeroId));
