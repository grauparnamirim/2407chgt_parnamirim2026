...
router.get('/impressoras', autenticar, operational,async  (req, res) => {
  const db = getDb();
  const rows = req.usuario.perfil === 'admin'
    ? (await db.prepare('SELECT * FROM impressoras').all())
    : (await db.prepare('SELECT * FROM impressoras WHERE unidade_id = ?').all(req.usuario.unidade_id));
  res.json(await Promise.all(rows.map(r => enrichPrintData(db, r))));
});

// Cadastra uma nova impressora
router.post('/impressoras', autenticar, operational,async  (req, res) => {
  ...
  const result = (await db.prepare(`INSERT INTO impressoras (nome, mac, ip_endereco, tipo, usuario_id, setor_id, local_id, unidade_id, ativo, contagem_atual, fabricante, modelo, num_serie, observacoes, criado_em, atualizado_em) VALUES (?,?,?,?,?,?,?,?,1,?,?,?,?,?,?,?)`).run(
...
router.put('/impressoras/:id', autenticar, operational,async  (req, res) => {
  const db = getDb();
  const item = (await db.prepare('SELECT * FROM impressoras WHERE id = ?').get(id(req.params.id)));
  ...
  if (sets.length) { sets.push('atualizado_em = ?'); vals.push(now()); vals.push(item.id); (await db.prepare(`UPDATE impressoras SET ${sets.join(', ')} WHERE id = ?`).run(...vals)); }
  res.json({ sucesso: true, mensagem: 'Impressora atualizada!' });
});

// Remove uma impressora
router.delete('/impressoras/:id', autenticar, operational,async  (req, res) => {
  (await getDb().prepare('DELETE FROM impressoras WHERE id = ?').run(id(req.params.id)));
  res.json({ sucesso: true, mensagem: 'Impressora removida!' });
});

// Ativa ou desativa uma impressora
router.put('/impressoras/:id/status', autenticar, operational,async  (req, res) => {
  (await getDb().prepare('UPDATE impressoras SET ativo = ? WHERE id = ?').run(req.body.ativo ? 1 : 0, id(req.params.id)));
  res.json({ sucesso: true, mensagem: 'Status atualizado!' });
});

// Lista leituras mensais de contagem de impressão
router.get('/leituras', autenticar, operational,async  (req, res) => {
  const db = getDb();
  const rows = req.usuario.perfil === 'admin'
    ? (await db.prepare('SELECT l.*, i.tipo, i.mac, i.ip_endereco AS ip, i.nome AS impressora_nome, s.nome AS setor_nome FROM leituras_mensais l JOIN impressoras i ON l.impressora_id = i.id LEFT JOIN setores s ON i.setor_id = s.id').all())
    : (await db.prepare('SELECT l.*, i.tipo, i.mac, i.ip_endereco AS ip, i.nome AS impressora_nome, s.nome AS setor_nome FROM leituras_mensais l JOIN impressoras i ON l.impressora_id = i.id LEFT JOIN setores s ON i.setor_id = s.id WHERE i.unidade_id = ?').all(req.usuario.unidade_id));
  res.json(rows);
});

// Registra uma nova leitura mensal de contagem
router.post('/leituras', autenticar, operational,async  (req, res) => {
  const db = getDb();
  const { impressora_id, contagem, observacoes } = req.body;
  const result = (await db.prepare('INSERT INTO leituras_mensais (impressora_id, criado_por, contagem, observacoes, criado_em) VALUES (?, ?, ?, ?, ?)').run(
...
router.get('/parametros-impressao', autenticar, operational,async  (req, res) => {
  const rows = (await getDb().prepare('SELECT * FROM parametros_impressao').all());
  const params = {};
...
router.put('/parametros-impressao', autenticar, operational,async  (req, res) => {
  const db = getDb();
  (await db.prepare('DELETE FROM parametros_impressao').run());
  const insert = db.prepare('INSERT INTO parametros_impressao (chave, valor) VALUES (?, ?)');
  await Promise.all(Object.entries(req.body || {}).map(([chave, valor]) => insert.run(chave, valor)));
  res.json({ sucesso: true, mensagem: 'Parâmetros atualizados!' });
});

// Retorna relatório mensal de impressão
router.get('/relatorios/impressao/mensal', autenticar, operational,async  (req, res) => {
  const db = getDb();
  const mes = Number(req.query.mes);
  const ano = Number(req.query.ano);
  const paramsRows = (await db.prepare('SELECT * FROM parametros_impressao').all());
  const parametros = {};
...
async function enrichPrintData(db, p) {
  return {
    ...p, ip: p.ip_endereco || '',
    usuario_nome: (await db.prepare('SELECT nome FROM usuarios WHERE id = ?').get(p.usuario_id))?.nome || '',
    setor_nome: (await db.prepare('SELECT nome FROM setores WHERE id = ?').get(p.setor_id))?.nome || '',
    local_nome: (await db.prepare('SELECT nome FROM locais WHERE id = ?').get(p.local_id))?.nome || ''
  };
}
