...
async function requireTicket(req, res, next) {
  const db = getDb();
  const ticket = (await db.prepare('SELECT * FROM chamados WHERE id = ?').get(id(req.params.id)));
...
async function requireStatusPermission(req, res, next) {
  const ticket = (await getDb().prepare('SELECT * FROM chamados WHERE id = ?').get(id(req.params.id)));
  if (!ticket) return res.status(404).json({ erro: 'Chamado não encontrado.' });
...
async function enrichChamado(db, c) {
  const bem = c.bem_id ? (await db.prepare('SELECT * FROM computadores WHERE id = ?').get(c.bem_id)) : null;
  return {
    ...c,
    ...
    fornecedor_nome: c.fornecedor_id ? (await db.prepare('SELECT nome FROM fornecedores WHERE id = ?').get(c.fornecedor_id))?.nome || '' : '',
    local_nome: c.local_id ? (await db.prepare('SELECT nome FROM locais WHERE id = ?').get(c.local_id))?.nome || '' : '',
    bem_nome: bem?.patrimonio || '',
    bem_patrimonio: bem?.patrimonio || '',
    bem_tipo: bem?.tipo || '',
    bem_fabricante: bem?.fabricante || '',
    bem_modelo: bem?.modelo || '',
    bem_local: bem?.local_id ? (await db.prepare('SELECT nome FROM locais WHERE id = ?').get(bem.local_id))?.nome || '' : '',
    tempo_espera_ms: 0
  };
}
...
router.post('/chamados', autenticar,async  (req, res) => {
  ...
  if (bemId) {
    const bem = (await db.prepare('SELECT * FROM computadores WHERE id = ?').get(bemId));
  ...
  const insert = await db.transaction(async () => {
    const result = (await db.prepare(`INSERT INTO chamados (titulo, descricao, usuario_id, tecnico_id, unidade_id, subcategoria_id, local_id, bem_id, status, criado_em, atualizado_em)
    ...
    const tecnico = (await db.prepare(`SELECT u.id FROM usuarios u
    ...
    if (tecnico) {
      (await db.prepare('UPDATE chamados SET tecnico_id = ? WHERE id = ?').run(tecnico.id, chamadoId));
    }
    return chamadoId;
  });
...
router.get('/chamados', autenticar,async  (req, res) => {
  const db = getDb();
  let rows;
  if (req.usuario.perfil === 'admin') {
    rows = (await db.prepare('SELECT * FROM chamados').all());
  } else if (req.usuario.perfil === 'usuario') {
    rows = (await db.prepare('SELECT * FROM chamados WHERE usuario_id = ?').all(req.usuario.id));
  } else {
    rows = (await db.prepare('SELECT * FROM chamados WHERE unidade_id = ?').all(req.usuario.unidade_id));
  }
  if (req.query.status) {
    const statuses = String(req.query.status).split(',');
    rows = rows.filter(c => statuses.includes(c.status));
  }
  res.json((await Promise.all(rows.map(c => enrichChamado(db, c)))).sort((a, b) => b.id - a.id));
});

// Altera o status de um chamado e registra notificação da mudança
router.put('/chamados/:id/status', autenticar, requireStatusPermission, operational,async  (req, res) => {
  ...
  const db = getDb();
  const before = (await db.prepare('SELECT * FROM chamados WHERE id = ?').get(id(req.params.id)));
  (await db.prepare('UPDATE chamados SET status = ?, motivo = ?, fornecedor_id = ?, tecnico_id = COALESCE(tecnico_id, ?), atualizado_em = ? WHERE id = ?').run(status, String(req.body.motivo || '').trim() || null, validId(req.body.fornecedor_id) ? id(req.body.fornecedor_id) : null, req.usuario.id, now(), id(req.params.id)));
  (await db.prepare('INSERT INTO notificacoes_log (chamado_id, usuario_id, alterado_por, status_anterior, status_novo, enviada_em) VALUES (?, ?, ?, ?, ?, ?)').run(before.id, before.usuario_id, req.usuario.id, before.status, status, now()));
  ...
  if (status === 'Resolvido' && before.bem_id) {
    const jaRegistrada = (await db.prepare('SELECT 1 FROM manutencoes WHERE chamado_id = ?').get(before.id));
    if (!jaRegistrada) {
      const somaCustos = (await db.prepare('SELECT COALESCE(SUM(valor), 0) AS total FROM custos_chamado WHERE chamado_id = ?').get(before.id)).total || 0;
      (await db.prepare(`INSERT INTO manutencoes (bem_id, tipo, categoria_servico_id, nome_servico, descricao, data_prevista, status, custo, tecnico_responsavel_id, criado_em, atualizado_em, data_realizada_em, chamado_id)
...
router.put('/chamados/:id/reabrir', autenticar, requireStatusPermission,async  (req, res) => {
  if (req.chamado.status !== 'Resolvido') return res.status(400).json({ erro: 'Apenas chamados resolvidos podem ser reabertos.' });
  (await getDb().prepare('UPDATE chamados SET status = ?, atualizado_em = ? WHERE id = ?').run('Reaberto', now(), id(req.params.id)));
  res.json({ sucesso: true, mensagem: 'Chamado reaberto com sucesso!' });
});

// Atualiza título, descrição e local de um chamado
router.put('/chamados/:id', autenticar, requireTicket, operational,async  (req, res) => {
  (await getDb().prepare('UPDATE chamados SET titulo = ?, descricao = ?, local_id = ?, atualizado_em = ? WHERE id = ?').run(
...
// Exclui um chamado
router.delete('/chamados/:id', autenticar, requireTicket, operational,async  (req, res) => {
  (await getDb().prepare('DELETE FROM chamados WHERE id = ?').run(id(req.params.id)));
  res.json({ sucesso: true, mensagem: 'Chamado excluído com sucesso!' });
});

// Retorna o histórico de alterações de status de um chamado
router.get('/chamados/:id/historico', autenticar, requireTicket,async  (req, res) => {
  const db = getDb();
  const chamado = await enrichChamado(db, (await db.prepare('SELECT * FROM chamados WHERE id = ?').get(id(req.params.id))));
  const historico = await Promise.all((await db.prepare('SELECT * FROM notificacoes_log WHERE chamado_id = ?').all(id(req.params.id))).map(async x => ({
    ...x, alterado_por_nome: (await db.prepare('SELECT nome FROM usuarios WHERE id = ?').get(x.alterado_por))?.nome || ''
  })));
  res.json({ chamado, historico });
});

// Retorna detalhes completos de um chamado com dados enriquecidos
router.get('/chamados/:id/detalhes', autenticar, requireTicket,async  (req, res) => {
  const db = getDb();
  res.json(await enrichChamado(db, (await db.prepare('SELECT * FROM chamados WHERE id = ?').get(id(req.params.id)))));
});

// Lista comentários de um chamado com dados do autor
router.get('/chamados/:id/comentarios', autenticar, requireTicket,async  (req, res) => {
  const db = getDb();
  const comentarios = await Promise.all((await db.prepare('SELECT * FROM comentarios WHERE chamado_id = ? ORDER BY id').all(id(req.params.id))).map(async x => ({
    ...x,
    autor_nome: (await db.prepare('SELECT nome FROM usuarios WHERE id = ?').get(x.usuario_id))?.nome || '',
    autor_perfil: (await db.prepare('SELECT perfil FROM usuarios WHERE id = ?').get(x.usuario_id))?.perfil || ''
  })));
  res.json(comentarios);
});

// Adiciona um comentário a um chamado
router.post('/chamados/:id/comentarios', autenticar, requireTicket,async  (req, res) => {
  const texto = String(req.body.texto || '').trim();
  if (!texto) return res.status(400).json({ erro: 'O comentário não pode estar vazio.' });
  (await getDb().prepare('INSERT INTO comentarios (chamado_id, usuario_id, texto, criado_em) VALUES (?, ?, ?, ?)').run(id(req.params.id), req.usuario.id, texto, now()));
